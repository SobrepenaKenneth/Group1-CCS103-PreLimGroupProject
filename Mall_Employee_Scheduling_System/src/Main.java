...
public class Main {
	...
	public static void AssignShift() {
		...
		// Check if timeSlot of selected shift has available spaces.)
		
		for(int i = 0; i < shiftSchedule[targetSheet][targetRow].length; i++) {
			// If a matching String is found in the row:
			if(shiftSchedule[targetSheet][targetRow][i].equals(shiftEntry)) {
				System.out.println("> " + employees[employeeRow][1] + " is already assigned to this time slot!");
				break;
			} 
			// If the slot is empty.
			if(shiftSchedule[targetSheet][targetRow][i] != null &&
			shiftSchedule[targetSheet][targetRow][i].equals("-")) {
				...
				break;
			} else if((i+1) == shiftSchedule[targetSheet][targetRow].length)
